  ...
  try {
    ...
    app.innerHTML = `
      <div class="card center">
        ...
        <span class="badge warn">Pending your lecturer's confirmation</span>
        <button class="btn" id="backBtn" style="margin-top:20px;">Done</button>
      </div>`;
  ...
  list.innerHTML = rows
    .map((r) => {
      const badge =
        ...
        r.status === 'rejected' ? '<span class="badge err">Rejected</span>' :
        '<span class="badge warn">Pending</span>';
      return `<div class="list-item">
        <div style="flex:1"><strong>${escapeHtml(r.code)}</strong> — ${escapeHtml(r.title)} ${badge}</div>
  ...
  async function refreshList() {
    ...
    listEl.innerHTML = rows
      .map((r) => {
        ...
        return `<div class="list-item" data-att-id="${r.id}">
          <img class="avatar" src="${r.photoUrl || placeholderAvatar(r.fullName)}" />
          <div style="flex:1">
            <strong>${escapeHtml(r.fullName)}</strong> ${badge}
            <div class="muted">${escapeHtml(r.idNumber)} · ${new Date(r.scannedAt + 'Z').toLocaleTimeString()}</div>
          </div>
          <div class="row" style="flex:0 0 auto;gap:6px;">
            <button class="btn small secondary" data-action="approved" data-id="${r.id}">Accept</button>
            <button class="btn small danger" data-action="rejected" data-id="${r.id}">Reject</button>
          </div>
        </div>`;
      })
      .join('');
    ...
    listEl.querySelectorAll('button[data-action]').forEach((btn) => {
      btn.onclick = async () => {
        try {
          await api(`/attendance/${btn.dataset.id}/status`, {
            method: 'PATCH',
            body: { status: btn.dataset.action },
          });
          refreshList();
        } catch (e) { toast(e.message, 'err'); }
      };
...
async function renderLecturerReports() {
  ...
  app.innerHTML = `
    <h1>Reports</h1>
    <p class="subtitle">Download a PDF attendance register for each course, organized by lecture date and time.</p>
    <div id="list"></div>
  `;
  const list = document.getElementById('list');
    ...
    .map(
      (c) => `<div class="card">
        <div class="list-item">
          ...
          <a class="btn small" href="/api/courses/${c.id}/report.pdf" target="_blank">Download PDF</a>
        </div>
      </div>`
    )
    .join('');
