...
async function renderScanner() {
  ...
  async function handleCode(text) {
    ...
    // QR payload includes a fresh unique id every refresh to prevent stale scans.
    locked = true;
    statusEl.textContent = 'Verifying…';
    try {
      const geo = await getGeoOrNull();
      if (geo && geo.accuracy > 80) {
        toast('Location is too inaccurate. Move to a clearer area and try again.', 'err');
        statusEl.textContent = 'Scanning…';
        locked = false;
        return;
      }
      const result = await api('/attendance/scan', {
        method: 'POST',
        body: { sid: payload.sid, tok: payload.tok, ts: payload.ts, lat: geo?.lat, lng: geo?.lng, accuracy: geo?.accuracy },
      });
      stopCamera();
      app.innerHTML = `
...
async function renderLecturerSession() {
  ...
  app.innerHTML = `
    ...
    <p class="subtitle">Generates a fresh QR code every 30 seconds so it can't be screenshotted and reused.</p>
    <div class="card" id="setupCard">
      ...
      <div class="row" id="radiusRow" style="display:none;">
        <div><label>Radius (km)</label><input id="radius" type="number" value="0.05" min="0.01" step="0.01" /></div>
      </div>
  ...
  document.getElementById('startBtn').onclick = async () => {
    try {
      ...
      if (document.getElementById('useGeo').checked) {
        ...
        if (geo.accuracy > 80) return toast('Location is too inaccurate. Move to a clearer area and try again.', 'err');
        lat = geo.lat; lng = geo.lng;
        const radiusKm = Number(document.getElementById('radius').value) || 0.05;
        radiusM = Math.max(radiusKm * 1000, 25);
      }
